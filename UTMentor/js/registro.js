...
(() => {
  ...
  // utilidades
  const open = (el) => { el?.classList.add('is-open'); el?.setAttribute('aria-hidden', 'false'); };
  const close = (el) => { el?.classList.remove('is-open'); el?.setAttribute('aria-hidden', 'true'); };
  const bindBackdropClose = (el) => {
    el?.addEventListener('click', (e) => {
      if (e.target.classList.contains('modal__backdrop') || e.target.hasAttribute('data-close')) close(el);
    });
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') close(el); });
  };
...
(function(){
  ...
  function showCardForm(show){
    if(show){ cardBlock.removeAttribute('hidden'); }
    else     { cardBlock.setAttribute('hidden',''); }
  }
  showCardForm(true);

  methods.forEach(btn=>{
    btn.addEventListener('click', ()=>{
      methods.forEach(b=>{ b.classList.remove('is-selected'); b.setAttribute('aria-selected','false'); });
      ...
      if(m === 'card' || m === 'mastercard'){ showCardForm(true); }
      else { showCardForm(false); }
    });
  });
